...
// รับค่า Firebase config จาก window.FIREBASE_CONFIG ที่ตั้งไว้ใน HTML
// ถ้าไม่มีก็ใช้ค่า default (ปิด Firebase)
const FB_CFG=(typeof window!=="undefined"&&window.FIREBASE_CONFIG)||{
  apiKey:"YOUR_API_KEY",
...
// ── Sound Effects ─────────────────────────────────────────
function useSounds(){
  const safeExec=(fn)=>{try{fn(getAC());}catch(e){}};
  const playTone=(ac,freq,type,vol,dur,delay=0,fEnd=null)=>{
  ...
  return{
    // UI clicks
    click:()=>safeExec(ac=>{playTone(ac,1200,"sine",0.06,0.05);playTone(ac,800,"sine",0.03,0.07,0.02);}),
    // Place card on board
    place:()=>safeExec(ac=>{
      playTone(ac,320,"sine",0.14,0.1,0,55);           // thud
      playTone(ac,1000,"sine",0.04,0.04,0.01);          // tap click
      noise(ac,0.06,0.06,500,0.0);               // paper flutter
    }),
    // Return card to hand
    ret:()=>safeExec(ac=>{playTone(ac,600,"sine",0.07,0.09,0,350);noise(ac,0.04,0.08,800);}),
    // Draw operator
    draw:()=>safeExec(ac=>{
      noise(ac,0.1,0.12,200,0);                  // shuffle
      playTone(ac,700,"sine",0.05,0.07,0.05);
      playTone(ac,900,"sine",0.04,0.05,0.08);
    }),
    // ✓ Correct equation
    correct:()=>safeExec(ac=>{
      [[523,0],[659,0.09],[784,0.18],[1047,0.27],[1319,0.38]].forEach(([f,d])=>{
        playTone(ac,f,"triangle",0.12,0.4,d);
      });
    }),
    ...
    combo:(n)=>safeExec(ac=>{
      ...
      [[base,0],[base*1.25,0.07],[base*1.5,0.14],[base*2,0.22],[base*2.5,0.32]].slice(0,n).forEach(([f,d])=>{
        playTone(ac,f,"triangle",0.1+n*0.01,0.35,d);
      });
    }),
    // ✗ Wrong answer
    wrong:()=>safeExec(ac=>{
      playTone(ac,200,"sawtooth",0.12,0.35,0,60);
      playTone(ac,150,"square",0.06,0.2,0.05);
      noise(ac,0.08,0.15,100,0.02);
    }),
    // Event card reveal
    event:()=>safeExec(ac=>{
      [[370,0],[466,0.12],[554,0.26],[740,0.42]].forEach(([f,d])=>playTone(ac,f,"triangle",0.09,0.5,d));
      noise(ac,0.06,0.3,1000,0.1);
    }),
    // Pass turn
    pass:()=>safeExec(ac=>{playTone(ac,440,"sine",0.07,0.15,0,300);playTone(ac,330,"sine",0.05,0.18,0.08);}),
    // Round goal completed
    goalDone:()=>safeExec(ac=>{
      [[880,0],[1109,0.08],[1318,0.16],[1760,0.26],[2217,0.38]].forEach(([f,d])=>playTone(ac,f,"triangle",0.1,0.38,d));
    }),
    // Achievement unlocked
    achievement:()=>safeExec(ac=>{
      [[523,0],[659,0.06],[784,0.12],[1047,0.18],[1319,0.25],[1568,0.33]].forEach(([f,d])=>playTone(ac,f,"triangle",0.09,0.5,d));
      noise(ac,0.08,0.4,2000,0.1);
    }),
    // Timer tick (hard mode)
    timer:()=>safeExec(ac=>{playTone(ac,1200,"square",0.04,0.04);}),
    timerEnd:()=>safeExec(ac=>{playTone(ac,440,"sawtooth",0.12,0.6,0,200);}),
    // Combine digits
    combine:()=>safeExec(ac=>{
      playTone(ac,800,"sine",0.08,0.12,0,1200);     // rising sweep
      playTone(ac,400,"sine",0.06,0.08,0.05);
      playTone(ac,1600,"triangle",0.05,0.1,0.1);    // sparkle
    ...
    reroll:()=>safeExec(ac=>{
      [0,0.04,0.09,0.15,0.22].forEach(d=>{
        ...
        playTone(ac,200+d*300,"sine",0.07,0.05,d);
      });
    }),
    // New round transition
    newRound:()=>safeExec(ac=>{
      [[330,0],[440,0.1],[554,0.2]].forEach(([f,d])=>playTone(ac,f,"triangle",0.08,0.4,d));
    }),
    // Game over
    gameOver:()=>safeExec(ac=>{
      [[262,0],[330,0.2],[392,0.4],[523,0.65],[392,1.0],[330,1.35]].forEach(([f,d])=>playTone(ac,f,"triangle",0.11,1.0,d));
    }),
    // Start game
    start:()=>safeExec(ac=>{
      [[392,0],[523,0.12],[659,0.22],[784,0.34],[1047,0.48]].forEach(([f,d])=>playTone(ac,f,"triangle",0.1,0.55,d));
    }),
  };
}
...
export default function NumberEquationGame(){
  ...
  // ── Sync Firebase leaderboard เมื่อเปิดหน้า lb หรือ comp_end ──
  useEffect(()=>{
    if((phase==="lb"||(phase==="comp_end"&&showLB))&&FB_ON){
      const unsub=fbListen(rows=>{setLeaderboard(rows);});
      return unsub; // cleanup listener เมื่อออกจากหน้า
    }
  },[phase,showLB]);
  ...
  if(phase==="intro")return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,background:"radial-gradient(ellipse at 50% -10%,#0A1830 0%,#050A12 55%,#030609 100%)"}}>
      <div style={{maxWidth:300,width:"100%",textAlign:"center"}}>
        ...
        <button onClick={()=>{setPhase("lb");setShowLB(true);if(FB_ON){fbListen(rows=>{setLeaderboard(rows);});}else{setLeaderboard(getLB());}}} style={{width:"100%",marginTop:10,padding:"12px 0",borderRadius:14,fontWeight:700,fontSize:15,background:"transparent",color:G.gold+"80",border:`1px solid ${G.gold}30`,cursor:"pointer",transition:"all .15s"}}>🏆 ตารางคะแนน</button>
      </div>
    </div>
  );
    ...
    <div style={{
      ...
      {phase==="lb"&&<div style={{position:"fixed",inset:0,background:G.bg,display:"flex",flexDirection:"column",zIndex:200,maxWidth:520,margin:"0 auto"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"8px 12px"}}>
          {leaderboard.length===0&&<div style={{color:"#FFFFFF20",fontSize:14,textAlign:"center",padding:"40px 0"}}>{FB_ON?"⏳ กำลังโหลดจาก Firebase...":"ยังไม่มีคะแนน"}<br/>{!FB_ON&&"เล่นโหมดแข่งขันแล้วบันทึกชื่อ"}</div>}
          {leaderboard.map((e,i)=>(
